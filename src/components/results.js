...
// Pagination reference: https://stackoverflow.com/questions/40232847/how-to-implement-pagination-in-reactjs
  ...
  handlePageClick(event) {
    ...
    // console.log('in results renderPageNumbers, handlePageClick, event: ', event);
    ...
    }, () => console.log('in results handleLeftPageClick, this.state.currentPage: ', this.state.currentPage)
    );
  }
    ...
    if (!flatsEmpty) {
      ...
      for (let i = 1; i <= Math.ceil(Object.keys(cards).length / cardsPerPage); i++) {
        ...
        if (i === Math.ceil(Object.keys(cards).length / cardsPerPage)) {
          console.log('in results renderPagination, if i === ... lastPage: ', i);
            lastPage = i;
        }
      }
